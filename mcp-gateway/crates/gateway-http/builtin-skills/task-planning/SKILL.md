...
8. If the same edit tool (`apply_patch` or `multi_edit_file`) fails three or more times in a row for the same `planningId`, the gateway may include `editFailureReminder`. Use it as a nudge to switch write strategy, inspect exact file content, simplify the patch, use the other edit tool, or fall back to a focused shell script when structured edits keep failing.
