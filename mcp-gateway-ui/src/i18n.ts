...
const translations = {
  zh: {
    ...
    skillsPathGuardRequired: "启用 Skill MCP 时必须开启访问边界",
    skillsWhitelistDirs: "允许访问目录",
    skillsWhitelistHint: "必填：添加终端可运行的绝对目录；命令和文件修改都必须留在这些目录内。",
    skillsAccessRequiredTitle: "需要填写允许访问目录",
    skillsAccessRequiredMsg: "你已启用内置 Skill MCP。请至少添加一个本机存在、终端可以进入并运行命令的绝对目录，例如你的项目目录；否则网关不会启动。",
    addFolderPath: "添加目录",
    browseFolder: "浏览",
  ...
  en: {
    ...
    skillsPathGuardRequired: "Access boundary is required while Skill MCP is enabled",
    ...
    skillsAccessRequiredTitle: "Allowed Directory Required",
    skillsAccessRequiredMsg: "Built-in Skill MCP is enabled. Add at least one existing absolute directory that the terminal can enter and run commands in, such as your project directory. The gateway will not start without it.",
